...
class CouchDbServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->app->singleton('couchdb_auth', function($app) {
            $baseUrl = sprintf('http://%s:%s@%s/%s/', urlencode(env('COUCHDB_USERNAME')), urlencode(env('COUCHDB_PASSWORD')), env('COUCHDB_HOST'), env('COUCHDB_AUTH_DATABASE'));
            $client = new Client(['base_uri' => $baseUrl, 'headers' => [ 'Content-Type' => 'application/json']]);
